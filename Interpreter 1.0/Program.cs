...
namespace Interpreter
{
    ...
    {
        private string dir;
        private string src; // keeps the location of the source code and the folder of the important config files
        private bool exit;
        private Random R;
        private string date;
        ...
        private void Init() //Initialization function
        {
            ...
            {
                Console.WriteLine("\nThere was an error trying to find the settings. aborting.");
                Console.Read();
            ...
            string txt = File.ReadAllText(src + "\\ascii-art.txt");
        ...
        {
            Console.WriteLine("\n----------\nCanceling action...\n----------");
            Thread T = new Thread(() => LiftOff(dir));
            T.Start();
            Thread.CurrentThread.Abort();
        }
        private void LiftOff(string dir)
        {
            ...
            do // main runtime loop
            {
                ...
                if (input.ToLower().Contains("lean"))//Important lean code
                {
                    ...
                    if (R.Next(0, 5) == 1)
                    {
                        Console.WriteLine(File.ReadAllText(src + "\\lean.txt") + "\n\n");
                    }
                    else
                    {
                        Console.WriteLine(File.ReadAllText(src + "\\lean!!!!!.txt") + "\n\n");
                ...
                if (input.IndexOf((char)MultiComType.Chain) != -1)
                {
                ...
                else if (input.IndexOf((char)MultiComType.Pipe) != -1)
                {
        ...
        static void Main(string[] args)
        {
            ...
            P.src = Directory.GetCurrentDirectory(); // we initialize fld here to make sure it stays the same even after a cancel event.
        ...
        private string Interpret(string[] coms, int i)
        {
            if (i == coms.Length - 1)
            {
                ...
                {
                    F = GetFuncNA(coms[i].Substring(0, coms[i].IndexOf(' ')));
                }
                if (!coms[i].Contains(" "))
                {
                    F2 = GetFunc(coms[i]);
                ...
                {
                    F2 = GetFunc(coms[i].Substring(0, coms[i].IndexOf(' ')));
                }
            ...
            else if (i == 0)
            {
                ...
                {
                    Console.WriteLine(F(Interpret(coms, i + 1)));
                    return "";
                }
        ...
        private string ReadFile(string input)
        {
            ...
            string[] settings = File.ReadAllLines(src + "\\settings.txt");
            ...
            if (bool.Parse(settings[4]) && File.ReadAllBytes(filename).Length > int.Parse(settings[5]))
            {
                ...
                settings[0] = settings[0].Substring(0, settings[0].IndexOf('/') - 1);
                Console.ForegroundColor = StringToConsoleColor(settings[0]);
